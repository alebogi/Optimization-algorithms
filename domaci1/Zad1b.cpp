...
bool fun(int x1, int x2, int x3) {
	cnt++;
	if ((x1 + x2 + x3 + (711 - (x1 + x2 + x3)) == 711) && (x1 * x2 * x3 * (711 - (x1 + x2 + x3)) == 711000000))
		return true;
	else
		return false;
}
...
* S tim sto su nam poznate tri cene, a cetvrta je izrazena preko njih.
* Program je optimizovan. Prvobitni program je sadrzao 3 for petlje koje su isle od 1 do 708.
...
* permutacija, program je izmenjen.
* Bez obrzira na optimizaciju, program ce naci nekoliko permutacija.
*/
